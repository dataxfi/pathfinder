...
export function uniswapV2Query(address: string, amt: string, skipT0: number = 0, skipT1: number = 0, callT0: boolean = true, callT1: boolean = true) {
  ...
  const t0Match = `t0IsMatch: pairs(first:1000 skip:${skipT0} where:{token0_contains:"${address}", volumeUSD_gt:"100" reserve0_gt:"${amt}"}
  ${generalReq}`;

  const t1Match = `t1IsMatch: pairs(first:1000 skip:${skipT1} where:{token1_contains:"${address}", volumeUSD_gt:"100" reserve1_gt:"${amt}"}
  ${generalReq}`;
